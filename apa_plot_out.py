...
class APA_PLOT:
    def load_sum (self, sum_path, rd_fn):
        fp = sum_path + rd_fn
    ...
    def del_bads (self, dicts, bad_wib_fembchns):
    ...
    def enctp_sort_bywire (self, dicts,  wiretype="X" ) :
        sort_apa_wires = []
    ...
    def enctp_sort_byapaloc (self, dicts,  apaloc=0  ) :
        sort_wires = []
    ...
    def enctp_sort_byfemb (self, femb_dicts ) :
        sdicts = sorted(femb_dicts, key=lambda x:x["fembchn"])
        return sdicts
    
    def dict_filter (self, dicts, and_dnf =[["gain","250"]], or_dnf = [["gain","250"]], and_flg = True, or_flg = True  ) :
        if (and_flg):
            and_apa_wires = []
            for chi in dicts:
                for dn in and_dnf:
    ...
    def apa_sorted_by_wire(self, orgdicts, g="250", tp="05") :
        dicts = self.dict_filter(orgdicts, and_dnf =[["gain",g],  ["tp",tp] ], or_dnf = [["fpgadac_en", True], ["asicdac_en", True]] ) 
        xdicts = self.enctp_sort_bywire (dicts,  wiretype="X" ) 
        vdicts = self.enctp_sort_bywire (dicts,  wiretype="V" ) 
        udicts = self.enctp_sort_bywire (dicts,  wiretype="U" ) 
        return xdicts, vdicts, udicts
    
    def femb_sorted_by_wire(self, orgdicts, g="250", tp="05", wibno=0, fembno=0) :
        dicts =  self.dict_filter(orgdicts, and_dnf =[["gain",g],  ["tp",tp], ["wib", wibno], ["femb", fembno] ], or_dnf = [["fpgadac_en", True], ["asicdac_en", True]] ) 
        xdicts = self.enctp_sort_bywire (dicts,  wiretype="X") 
        vdicts = self.enctp_sort_bywire (dicts,  wiretype="V") 
        udicts = self.enctp_sort_bywire (dicts,  wiretype="U") 
        return xdicts, vdicts, udicts
    
    def find_a_chn(self, orgdicts, wibno=0, fembno=0, fembchn=0) :
        dicts = self.dict_filter(orgdicts, and_dnf =[["wib", wibno], ["femb", fembno], ["fembchn", fembchn] ], or_dnf = [["fpgadac_en", True], ["asicdac_en", True]] ) 
        return dicts
    
    def draw_results (self, dicts) :
        apachn=[]
        rms=[]
        hfrms=[]
        sfrms=[]
        ...
        asi_gain=[]
        unstk_ratio = []
        fembx_cnt, fembv_cnt, fembu_cnt = self.femb_xuv 
        for i in range(len(dicts)):
            if dicts[i]["wire"][0] == "X":
    ...
    ###############################################################################################################################
    ###############################################################################################################################
    def sub_rms_c_plot5 (self, ax, dicts, rms_cs="rms", cali_cs="fpg_gain" ) :
        for loc in self.fembs_on_apa:
            ...
            if subdicts != None:
                ...
                apachn, rms, hfrms, sfrms, ped, hfped, sfped, fpg_gain, asi_gain, unstk_ratio = self.draw_results (subdicts) 
                if (rms_cs=="rms" ):
                    plotrms = rms
                    t = "Raw Data"
                elif (rms_cs=="hfrms" ):
    ...
    def sub_rms_p_plot5 (self, ax, dicts, rms_cs="hfrms" ) :
        apachn = []
        apachn, rms, hfrms, sfrms, ped, hfped, sfped, fpg_gain, asi_gain, unstk_ratio = self.draw_results (dicts) 
        if (rms_cs=="rms" ):
            plotrms = rms
            t = "Raw Data"
        elif (rms_cs=="hfrms" ):
    ...
    def apa_plot5 (self, pp, orgdicts, title="APA ENC vs. Tp", rmstype="sfrms", calitype="fpg_gain" , gs=["250", "140"], tps=["05", "10", "20", "30"]) : 
        ...
        for g in gs:
            for tp in tps:
                xd,vd,ud=self.apa_sorted_by_wire(orgdicts, g=g, tp=tp) 
                self.rms_self.c_plot5 (pp, xd, title="RMS Compare" ) 
                self.rms_self.c_plot5 (pp, vd, title="RMS Compare" ) 
                self.rms_c_plot5 (pp, ud, title="RMS Compare" ) 
    
    def rms_c_plot5 (self, pp, dicts, title="RMS Compare" ) :
        ...
        self.sub_rms_c_plot5 (ax1, dicts , rms_cs="rms", cali_cs="fpg_gain") 
        self.sub_rms_c_plot5 (ax2, dicts , rms_cs="hfrms", cali_cs="fpg_gain") 
        self.sub_rms_c_plot5 (ax3, dicts , rms_cs="sfrms", cali_cs="fpg_gain") 
        self.sub_rms_p_plot5 (ax4, dicts, rms_cs="hfrms" ) 
        self.sub_rms_p_plot5 (ax5, dicts, rms_cs="sfrms" ) 
    ...
    ###############################################################################################################################
    ###############################################################################################################################
    def draw_enctp_plot0(self, wdicts, calitype = "fpg_gain"):
        dr = self.draw_results (wdicts) 
        drchns = len(dr[0])
    ...
    def sub_enctp_plot0 (self, ax, g,calitype, tp_us, xchns, xenc_tps, vchns, venc_tps, uchns, uenc_tps, title="APA ENC vs. Tp" , note="Raw data") :
        if (calitype == "fpg_gain"):
            dactype = "FPGA DAC"
        else:
            dactype = "ASIC DAC"
    ...
    def plot0_overall_enc (self, pp, orgdicts, title="APA ENC vs. Tp", calitype="fpg_gain", sfhf = "sf", gs =["250", "140"],  tps=["05", "10", "20", "30"] ) :
        ...
        for g in gs:
            ...
            if ( g == "250" ):
                self.sub_enctp_plot0 (ax1, g, calitype, tp_us, xchns, xenc_tps, vchns, venc_tps, uchns, uenc_tps, note="Raw data") 
                if (sfhf == "sf"):
                    self.sub_enctp_plot0 (ax3, g, calitype, tp_us, xchns, xsfenc_tps, vchns, vsfenc_tps, uchns, usfenc_tps, note="Stuck Free") 
                else:
                    self.sub_enctp_plot0 (ax3, g, calitype, tp_us, xchns, xhfenc_tps, vchns, vhfenc_tps, uchns, uhfenc_tps, note="HPF data") 
            else:
                self.sub_enctp_plot0 (ax2, g, calitype, tp_us, xchns, xenc_tps, vchns, venc_tps, uchns, uenc_tps, note="Raw data") 
                if (sfhf == "sf"):
                    self.sub_enctp_plot0 (ax4, g, calitype, tp_us, xchns, xsfenc_tps, vchns, vsfenc_tps, uchns, usfenc_tps, note="Stuck Free") 
                else:
                    self.sub_enctp_plot0 (ax4, g, calitype, tp_us, xchns, xhfenc_tps, vchns, vhfenc_tps, uchns, uhfenc_tps, note="HPF data") 
    ...
    def plot1_chns_enc (self, pp, orgdicts, title="APA ENC s. Tp", wiretype="X",  cali_cs="fpg_gain", rms_cs = "raw", g="250" ,  tps=["05", "10", "20", "30"]
        ...
        uenc_tps=[]
        for tp in tps:
            dicts_xvu = self.dict_filter(orgdicts, and_dnf =[["gain",g],  ["tp",tp] ], or_dnf = [["fpgadac_en", True], ["asicdac_en", True]] ) 
            dicts = self.enctp_sort_bywire (dicts_xvu,  wiretype=wiretype , ) 
            self.sub_chns_plot1 (ax1, dicts, rms_cs=rms_cs, cali_cs=cali_cs, ) 
            if ( tp == "05" ):
                self.sub_chns_hist_plot1 (ax2, dicts, rms_cs=rms_cs, cali_cs=cali_cs ) 
            elif ( tp == "10" ):
                self.sub_chns_hist_plot1 (ax3, dicts, rms_cs=rms_cs, cali_cs=cali_cs ) 
            elif ( tp == "20" ):
                self.sub_chns_hist_plot1 (ax4, dicts, rms_cs=rms_cs, cali_cs=cali_cs ) 
            elif ( tp == "30" ):
                self.sub_chns_hist_plot1 (ax5, dicts, rms_cs=rms_cs, cali_cs=cali_cs ) 
    ...
    def sub_chns_hist_plot1 (self, ax, dicts, rms_cs="rms", cali_cs="fpg_gain" ) :
        apachn = []
        ymax = 2000
        tp = int( dicts[0]["tp"]) / 10.0
        gain = int(dicts[0]["gain"]) / 10.0
    ...
    ###############################################################################################################################
    ###############################################################################################################################
    def sub_ped_plot2 (self, ax, dicts  ) :
        apachn = []
        for loc in self.fembs_on_apa:
            subdicts = self.enctp_sort_byapaloc (dicts,  apaloc=loc  ) 
            ymax = 4100
            ...
            if subdicts != None:
                ...
                wire_cnt = np.max(apachn) - np.min(apachn) + 1
                ax.text(5+(loc-1)*wire_cnt, ymax*0.90, "Dead", fontsize=8)
                if ( loc != 1):
                    ax.vlines((loc-1)*wire_cnt, 0, ymax, color='b',linestyles="dotted", linewidth=0.8)
        ax.set_ylim([0,ymax])
        #ax.set_xlim([np.min(apachn),np.max(apachn) ])
    ...
    def sub_ped_hist_plot2 (self, ax, dicts ) :
        apachn, rms, hfrms, sfrms, ped, hfped, sfped, fpg_gain, asi_gain, unstk_ratio = self.draw_results (dicts) 
        pedmean = np.mean(ped)
        pedstd = np.std(ped)
        N = len(ped)
    ...
    def plot2_peds (self, pp, orgdicts, title="Pedestals", g="250", tp="20"  ) :
        fig = plt.figure(figsize=(16,9))
        ...
        dicts_xvu = self.dict_filter(orgdicts, and_dnf =[["gain",g],  ["tp",tp] ], or_dnf = [["fpgadac_en", True], ["asicdac_en", True]] ) 
        dicts = self.enctp_sort_bywire (dicts_xvu,  wiretype = "X" ) 
        self.sub_ped_plot2 (ax1, dicts ) 
        self.sub_ped_hist_plot2 (ax4, dicts ) 
        dicts = self.enctp_sort_bywire (dicts_xvu,  wiretype = "V" ) 
        self.sub_ped_plot2 (ax2, dicts ) 
        self.sub_ped_hist_plot2 (ax5, dicts ) 
        dicts = self.enctp_sort_bywire (dicts_xvu,  wiretype = "U"  ) 
        self.sub_ped_plot2 (ax3, dicts) 
        self.sub_ped_hist_plot2 (ax6, dicts ) 
    ...
    ###############################################################################################################################
    ###############################################################################################################################
    def sub_gain_plot3 (self, ax, g, tp_us, xchns, xgain, vchns, vgain, uchns, ugain, title="Gain Measurement" , note="FPGA-DAC") :
    ...
    def plot3_overall_gain (self, pp, orgdicts, title="APA Gain Measurement", gs=["250", "140"], tps=["05", "10", "20", "30"]) :
        ...
        for g in gs:
            ...
            for tp in tps:
                tp_us.append(int(tp)/10.0)
                xd, vd, ud = self.apa_sorted_by_wire(orgdicts, g=g, tp=tp) 
                dr = self.draw_results (xd) 
                xchns = len(dr[0])
                xfpg_dac.append([np.mean(dr[7]), np.std(dr[7])] )
                xasi_dac.append([np.mean(dr[8]), np.std(dr[8])] )
    
                dr = self.draw_results (vd) 
                vchns = len(dr[0])
                vfpg_dac.append([np.mean(dr[7]), np.std(dr[7])] )
                vasi_dac.append([np.mean(dr[8]), np.std(dr[8])] )
    
                dr = self.draw_results (ud) 
                uchns = len(dr[0])
                ufpg_dac.append([np.mean(dr[7]), np.std(dr[7])] )
                uasi_dac.append([np.mean(dr[8]), np.std(dr[8])] )
     
            if ( g == "250" ):
                self.sub_gain_plot3 (ax1, g, tp_us, xchns, xfpg_dac, vchns, vfpg_dac, uchns, ufpg_dac, note="FPGA-DAC") 
                self.sub_gain_plot3 (ax2, g, tp_us, xchns, xasi_dac, vchns, vasi_dac, uchns, uasi_dac, note="ASIC-DAC") 
            else:
                self.sub_gain_plot3 (ax3, g, tp_us, xchns, xfpg_dac, vchns, vfpg_dac, uchns, ufpg_dac, note="FPGA-DAC") 
                self.sub_gain_plot3 (ax4, g, tp_us, xchns, xasi_dac, vchns, vasi_dac, uchns, uasi_dac, note="ASIC-DAC") 
    ...
    def plot4_chns_gain (self, pp, orgdicts, title="Gain Measurement", wiretype="X", g="250", cali_cs="fpg_gain" ): #enctype, raw, hf, sf
        ...
        for tp in tps:
            dicts_xvu = self.dict_filter(orgdicts, and_dnf =[["gain",g],  ["tp",tp] ], or_dnf = [["fpgadac_en", True], ["asicdac_en", True]] ) 
            dicts = self.enctp_sort_bywire (dicts_xvu,  wiretype=wiretype  ) 
            self.sub_gain_plot4 (ax1, dicts, cali_cs=cali_cs ) 
            if ( tp == "05" ):
                self.sub_hist_gain_plot4 (ax2, dicts, cali_cs=cali_cs ) 
            elif ( tp == "10" ):
                self.sub_hist_gain_plot4 (ax3, dicts, cali_cs=cali_cs ) 
            elif ( tp == "20" ):
                self.sub_hist_gain_plot4 (ax4, dicts, cali_cs=cali_cs ) 
            elif ( tp == "30" ):
                self.sub_hist_gain_plot4 (ax5, dicts, cali_cs=cali_cs ) 
    ...
    def sub_gain_plot4 (self, ax, dicts, cali_cs="fpg_gain"  ) :
        apachn = []
        for loc in self.fembs_on_apa:
            subdicts = self.enctp_sort_byapaloc (dicts,  apaloc=loc  ) 
            ymax = 500
    ...
    def sub_hist_gain_plot4 (self, ax, dicts,  cali_cs="fpg_gain" ) :
        ymax = 500 
        tp = int( dicts[0]["tp"]) / 10.0
        gain = int(dicts[0]["gain"]) / 10.0
        apachn, rms, hfrms, sfrms, ped, hfped, sfped, fpg_gain, asi_gain, unstk_ratio = self.draw_results (dicts) 
    ...
    def __init__(self):
        self.apa = 'ProtoDUNE'    
        self.femb_xuv = [48,40,40]
        self.fembs_on_apa = range(1,21,1)
        self.apachn_s = 0
        self.apachn_e = 0
