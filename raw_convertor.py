...
Last modified: Tue Dec 10 16:47:36 2019
"""
...
def raw_convertor_filtered(raw_data, smps, jumbo_flag=True):
    ...
    for i in range(len(flt_chn_data)):
        flt_chn_data[i] = hp_flt_applied(chn_data[i], fs = 2000000, passfreq = 1000, flt_order = 3)
...
def raw_convertor_peak(raw_data, smps, jumbo_flag=True):
    ...
    if (True):
        ...
        for chn in range(16):
            peakps = list(chn_peakp[chn])
            peakns = list(chn_peakn[chn])
            for i in range(rmi):
                peakps.remove(np.max(peakps))
                peakps.remove(np.min(peakps))
                peakns.remove(np.max(peakns))
                peakns.remove(np.min(peakns))
            chn_peakp[chn] = peakps
            chn_peakn[chn] = peakns
