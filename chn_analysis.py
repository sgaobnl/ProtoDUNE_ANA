...
#    return apainfo, wireinfo, feset_info, chn_noise_paras, chn_cali_paras
...
#    if (not mpout.empty() ):
#        chn_results = [mpout.get() for p in [p1,p2] ]
#    else:
#        "ERROR!, can't get this channel parameters, exit anway!"
#        exit()
#
#    if(len(chn_results) != 2):
#        "ERROR!, can't get this channel noise or calibration parameters, exit anway!"
#        exit()
#    else:
#        if (chn_results[0][0] == 1 ):
#            chn_noise_paras = chn_results[0][1]
#            chn_cali_paras = chn_results[1][1]
#        else:
#            chn_noise_paras = chn_results[1][1]
#            chn_cali_paras = chn_results[0][1]
#    p1.join()
#    p2.join()
...
##    mpout = Queue()
#    mpout = mp.Queue()
#    noise_a_chn_argvs = (mpout, rmsdata, chnno,fft_en, fft_s, fft_avg_cycle)
#    cali_a_chn_argvs  = (mpout, calidata, chnno, cap )
#    p1 = mp.Process(target=mp_noise_a_chn, args=noise_a_chn_argvs)
#    p2 = mp.Process(target=mp_cali_a_chn,  args=cali_a_chn_argvs)
#    p1.start()
#    p2.start()
...
#
##    if (not mpout.empty() ):
##k        chn_results = [mpout.get() for p in [p1] ]
#    print len(chn_results)
#
...
###    p1 = mp.Process(target=mp_noise_a_chn, args=noise_a_chn_argvs)
###    p2 = mp.Process(target=mp_cali_a_chn,  args=cali_a_chn_argvs)
### #   noise_a_chn_argvs = (rmsdata, chnno,fft_en, fft_s, fft_avg_cycle)
### #   cali_a_chn_argvs  = (calidata, chnno, cap )
### #   p1 = mp.Process(target=noise_a_chn, args=noise_a_chn_argvs)
### #   p2 = mp.Process(target=cali_a_chn,  args=cali_a_chn_argvs)
### #
###    p1.start()
###    p2.start()
