...
Last modified: Sun Mar 25 11:35:17 2018
"""
...
def mp_ana_a_asic(mpout, rms_rootpath, fpga_rootpath, asic_rootpath,  APAno = 4, \
    ...
    for chni in range(16):
        ...
        asic_results.append([apainfo, wireinfo, feset_info, rms ,ped ,hfrms ,hfped ,sfrms ,sfped  ,unstk_ratio, fpg_cali_flg, fpg_encperlsb, fpg_chninl, asi_cali_flg, asi_encperlsb, asi_chninl])
...
def chk_a_wib (rms_rootpath, fpga_rootpath, asic_rootpath, APAno = 4, rmsrunno = "run01rms", fpgarunno = "run01fpg", asicrunno = "run01asi",\
    ...
    for fembno in range(4):
        for asicno in range(8):
            ...
            if (len(files_cs) == 1):
                rms_files_flg = True
            files_cs = generate_rawpaths(fpga_rootpath, asic_rootpath, runno=fpgarunno, wibno=wibno,  fembno=fembno, chnno=asicno*16, gain=gain, tp=tp ) 
            if (len(files_cs) >= 1):
                fpg_files_flg = True
            files_cs = generate_rawpaths(fpga_rootpath, asic_rootpath, runno=asicrunno, wibno=wibno,  fembno=fembno, chnno=asicno*16, gain=gain, tp=tp ) 
            if (len(files_cs) >= 1):
                asi_files_flg = True
            ...
            if (not rms_files_flg):
                if (not (fpg_files_flg or asi_files_flg) )
                    break
        if rmsfiles_flg and (fpg_files_flg or asi_files_flg):
...
def ana_a_wib(rms_rootpath, fpga_rootpath, asic_rootpath, APAno = 4, rmsrunno = "run01rms",fpgarunno = "run01fpg", asicrunno = "run01asi",\
    ...
    out_path = rms_rootpath + "/" + "results/" + "APA%d_"%APAno + rmsrunno + "_" + fpgarunno + asicrunno +"/"
    ...
    out_fn = "APA%d"%APAno + "_WIB%d"%wibno + "_Gain%s"%gain + "_Tp%s"%tp+  "_" + rmsrunno + "_" + fpgarunno + asicrunno+ ".sum"
    fp = out_path + out_fn
    print fp
...
if __name__ == '__main__':
    ...
    for gain in gains: 
        for tp in tps:
            print "Gain = %2.1f mV/fC, "% (int(gain)/10.0) +  "Tp = %1.1f$\mu$s"% (int(tp)/10.0)  ]
            ana_a_apa(rms_rootpath, fpga_rootpath, asic_rootpath, APAno, rmsrunno, fpgarunno, asic_runno, gain, tp, jumbo_flag)
